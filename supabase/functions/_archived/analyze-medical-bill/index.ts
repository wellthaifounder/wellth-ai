...
`;
    ...
    if (!aiResponse.ok) {
      ...
      if (aiResponse.status === 429) {
        throw new Error(
          "AI rate limit exceeded. Please try again in a few minutes.",
        );
      }
    ...
    if (analysisResult.metadata) {
      ...
      if (
        serviceDate &&
        billDate &&
        new Date(serviceDate) > new Date(billDate)
      ) {
        validationWarnings.push(
          "Service date is after bill date - please verify dates",
        );
      }
      ...
      if (
        providerName &&
        ["Hospital", "Clinic", "Medical Center", "Billing Department"].some(
          (generic) => providerName === generic,
        )
      ) {
        validationWarnings.push(
          "Provider name appears generic - may need manual verification",
        );
      ...
      for (const field of Object.values(analysisResult.metadata)) {
        if (field && typeof field === "object" && "confidence" in field) {
          if (field.confidence < 0 || field.confidence > 1) {
            console.warn(
              `Invalid confidence score ${field.confidence}, clamping to 0-1`,
            );
    ...
    if (analysisResult.errors && analysisResult.errors.length > 0) {
      ...
      const calculatedTotal = analysisResult.errors.reduce(
        (sum, e) => sum + (e.potential_savings || 0),
        0,
      );
      ...
      if (Math.abs(calculatedTotal - reportedTotal) > 1) {
        console.warn(
          `Savings mismatch: calculated ${calculatedTotal}, reported ${reportedTotal}`,
        );
      ...
      const duplicates = analysisResult.errors.filter(
        (e) => e.error_type === "duplicate_charge",
      );
      for (const dup of duplicates) {
        if (dup.evidence?.duplicate_count && dup.evidence?.charge_amount) {
          const expectedSavings =
            dup.evidence.charge_amount * (dup.evidence.duplicate_count - 1);
          if (Math.abs(expectedSavings - dup.potential_savings) > 1) {
            console.warn(
              `Duplicate charge calculation error in: ${dup.description}`,
            );
          }
        }
      }
    ...
    if (
      ...
      analysisResult.confidence_score > 1
    ) {
      console.warn(
        `Invalid overall confidence ${analysisResult.confidence_score}, setting to 0.5`,
      );
    ...
    console.log(
      `Validation complete. ${validationWarnings.length} warnings generated.`,
    );
        ...
        .upsert(
          {
            ...
            extracted_date:
              metadata.service_date?.value || metadata.bill_date?.value || null,
