...
function extractProviderData(npiData: NPIResult) {
  ...
  const primaryTaxonomy =
    basic.taxonomies.find((t) => t.primary) || basic.taxonomies[0];
  ...
  try {
    ...
    if (batch_sync) {
      ...
      const { data: providers, error } = await supabase
        ...
        .or(
          "specialties_verified.is.null,specialties_verified.eq.false,data_last_updated.lt.now() - interval '30 days'",
        )
      ...
      if (provider) {
        ...
        console.log(
          `Provider with NPI ${npi_number} not found, fetching data...`,
        );
      ...
      return new Response(
        JSON.stringify({
          error:
            "Missing required parameter: npi_number, provider_id, or batch_sync",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    for (const provider of providersToSync) {
      try {
        ...
        if (!npiData) {
          results.failed++;
          results.errors.push(
            `NPI ${provider.npi_number}: Not found in registry`,
          );
          continue;
        }
        ...
        if (updateError) {
          results.failed++;
          results.errors.push(
            `Provider ${provider.id}: ${updateError.message}`,
          );
