...
const PHI_PATTERNS: PHIPattern[] = [
  ...
  },
  {
    type: "address_line",
    pattern:
      /\b\d+\s+[A-Za-z0-9\s,.-]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Court|Ct|Circle|Cir|Way|Place|Pl)\b/gi,
...
function detectPatternBasedPHI(text: string): {
  redactedText: string;
...
function redactAIDetectedPHI(
  text: string,
  phiSnippets: string[],
): { redactedText: string; detectedPHI: DetectedPHI[] } {
  let redactedText = text;
  const detectedPHI: DetectedPHI[] = [];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { redactedText: patternRedacted, detectedPHI: patternPHI } =
      detectPatternBasedPHI(text);
    ...
    if (use_ai) {
      ...
      console.log(
        `AI detection found ${aiSnippets.length} potential PHI snippets`,
      );

      if (aiSnippets.length > 0) {
        const { redactedText: aiRedacted, detectedPHI: aiPHI } =
          redactAIDetectedPHI(finalRedactedText, aiSnippets);
    ...
    const stats = {
      ...
      phi_by_type: allDetectedPHI.reduce(
        (acc, phi) => {
