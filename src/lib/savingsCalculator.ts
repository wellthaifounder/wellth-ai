...
export const calculateSavings = (data: CalculatorData): SavingsBreakdown => {
  ...
  if (accountType === "hsa") {
    ...
    taxExplanation =
      "HSA contributions are tax-deductible, and qualified withdrawals are tax-free.";
    ...
    taxLabel = `$${annualSpending.toLocaleString()} x ${taxBracket}% tax rate`;
    taxExplanation =
      "Opening an HSA or FSA could save you this amount in taxes each year.";
  }
  ...
  if (accountType === "hsa") {
    ...
    const futureValue =
      taxSavings * Math.pow(1 + ASSUMED_RETURN_RATE, ASSUMED_GROWTH_YEARS);
    ...
    growthExplanation =
      "HSA funds can be invested and grow tax-free. This projection assumes a 7% average annual return over 5 years.";
  } else if (accountType === "fsa") {
    growthLabel = "N/A for FSA";
    growthExplanation =
      "FSA funds must be used within the plan year (with possible grace period), so long-term investment growth doesn't apply.";
  } else {
    ...
    growthLabel = `$${taxSavings.toLocaleString()} x ${ASSUMED_RETURN_RATE * 100}% return x ${ASSUMED_GROWTH_YEARS} years`;
    growthExplanation =
      "If you opened an HSA, your tax savings could be invested and grow tax-free at an estimated 7% annual return.";
  }
