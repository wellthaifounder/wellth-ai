...
export const generateHSAMaximizerReport = async (
  ...
  const addPageHeader = (pageNum: number) => {
    ...
    pdf.text(
      `HSA Maximizer Blueprint | Page ${pageNum} of 10`,
      pageWidth / 2,
      12,
      { align: "center" },
    );
  ...
  const projectedContribution = Math.min(
    ...
    data.householdSize === 1
      ? HSA_LIMITS_2025.selfOnly
      : HSA_LIMITS_2025.family,
  );
  const years = 10;
  ...
  const investmentGrowth = Math.round(
    futureValue - projectedContribution * years,
  );
  ...
  pdf.setFontSize(16);
  pdf.text("I'm Wellbie, your personal HSA guide", pageWidth / 2, 42, {
    align: "center",
  });
  ...
  pdf.setFontSize(12);
  pdf.text("Here's the exciting part:", pageWidth / 2, 113, {
    align: "center",
  });
  pdf.setFontSize(34);
  pdf.setTextColor(brand.primary.r, brand.primary.g, brand.primary.b);
  pdf.text(`$${savings.total.toLocaleString()}`, pageWidth / 2, 128, {
    align: "center",
  });
  pdf.setFontSize(11);
  pdf.setTextColor(brand.secondary.r, brand.secondary.g, brand.secondary.b);
  pdf.text("in potential annual savings!", pageWidth / 2, 137, {
    align: "center",
  });
  ...
  pdf.setFontSize(11);
  pdf.text(
    `For ${data.householdSize > 1 ? "your household of " + data.householdSize : "you"}`,
    pageWidth / 2,
    167,
    { align: "center" },
  );
  pdf.text(
    `Based on $${data.monthlySpending.toLocaleString()}/month healthcare spending`,
    pageWidth / 2,
    177,
    { align: "center" },
  );
  ...
  pdf.setFontSize(9);
  pdf.text(`Created just for you on ${today}`, pageWidth / 2, pageHeight - 15, {
    align: "center",
  });
  if (userEmail) {
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "After analyzing your healthcare spending, I found some really exciting opportunities",
    20,
    yPos,
  );
  yPos += 7;
  pdf.text(
    "to put more money back in your pocket. Here's what caught my eye:",
    20,
    yPos,
  );
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `💡 Invest this & it could grow to $${Math.round(futureValue).toLocaleString()} in ${years} years (+$${investmentGrowth.toLocaleString()} growth)!`,
    25,
    yPos + 24,
  );
  ...
  pdf.setTextColor(brand.success.r, brand.success.g, brand.success.b);
  pdf.text(
    `$${savings.rewardsSavings.toLocaleString()}/year`,
    pageWidth - 25,
    yPos + 8,
    { align: "right" },
  );
  ...
  pdf.setTextColor(brand.success.r, brand.success.g, brand.success.b);
  pdf.text(
    `$${savings.timingSavings.toLocaleString()}/year`,
    pageWidth - 25,
    yPos + 8,
    { align: "right" },
  );
  ...
  pdf.setFontSize(20);
  pdf.text(`$${savings.total.toLocaleString()}`, pageWidth - 25, yPos + 10, {
    align: "right",
  });
  ...
  yPos += 10;
  pdf.text(
    "• The triple tax advantage of HSAs (it's like a financial superpower!)",
    25,
    yPos,
  );
  yPos += 7;
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("How I Crunched Your Numbers", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "Let me show you exactly where these savings come from. No magic, just smart strategy!",
    20,
    43,
  );
  ...
  pdf.text(
    `You spend about $${annualSpending.toLocaleString()} on healthcare each year`,
    25,
    yPos,
  );
  yPos += 7;
  ...
  if (data.hasHSA === "yes") {
    ...
    yPos += 7;
    pdf.text(
      `You're likely saving around ${(taxRate * 100).toFixed(0)}% in taxes right now`,
      25,
      yPos,
    );
    ...
    taxRate = 0.22;
    pdf.text(
      "You don't have an HSA yet - that's your biggest opportunity!",
      25,
      yPos,
    );
    yPos += 7;
    pdf.text(
      `Opening one could save you ${(taxRate * 100).toFixed(0)}% on every healthcare dollar`,
      25,
      yPos,
    );
  } else {
    ...
    yPos += 7;
    pdf.text(
      `I used a conservative ${(taxRate * 100).toFixed(0)}% estimate for your potential savings`,
      25,
      yPos,
    );
  }
  ...
  if (data.householdSize > 1) {
    pdf.text(
      `Plus, with ${data.householdSize} people in your household, that's ${householdMultiplier.toFixed(2)}x the opportunity`,
      25,
      yPos,
    );
    yPos += 7;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `💚 Tax Savings: $${savings.taxSavings.toLocaleString()}/year`,
    25,
    yPos,
  );
  ...
  const paymentMethodText =
    {
      credit:
        data.hasRewards === "yes"
          ...
          : "credit card (but no rewards yet)",
  ...
  if (data.paymentMethod === "credit") {
    if (data.hasRewards === "yes") {
      ...
      yPos += 7;
      pdf.text(
        "But we can boost that to 3% or more with the right card",
        25,
        yPos,
      );
    } else {
      ...
      pdf.text(
        "By switching to a rewards card, you could earn 3% back",
        25,
        yPos,
      );
    }
    ...
    pdf.text(
      "By using a rewards credit card, you could earn 2.5-3% back",
      25,
      yPos,
    );
  }
  ...
  pdf.setTextColor(100, 100, 100);
  pdf.text(
    `Math: $${annualSpending.toLocaleString()} × ${(rewardsRate * 100).toFixed(1)}% = $${calculatedRewards.toLocaleString()}`,
    25,
    yPos,
  );
  yPos += 7;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `💚 Rewards Potential: $${savings.rewardsSavings.toLocaleString()}/year`,
    25,
    yPos,
  );
  ...
  yPos += 7;
  pdf.text(
    "By timing these strategically, you can maximize deductible hits,",
    25,
    yPos,
  );
  yPos += 7;
  pdf.text(
    "align with FSA deadlines, and optimize your annual contributions",
    25,
    yPos,
  );
  yPos += 7;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `💚 Timing Savings: $${savings.timingSavings.toLocaleString()}/year`,
    25,
    yPos,
  );
  ...
  pdf.setFontSize(24);
  pdf.setFont(undefined, "bold");
  pdf.text("The HSA Triple Tax Advantage", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "Here's why HSAs are my favorite financial tool (and should be yours too!):",
    20,
    yPos,
  );
  yPos += 18;
  ...
  const taxAdvantages = [
    {
      ...
      desc: "Every dollar you put in reduces your taxable income. It's like getting a discount on your taxes!",
    },
    {
      emoji: "📈",
      title: "Tax-Free Growth",
      desc: "Invest your HSA funds and watch them grow without paying a penny in capital gains or dividend taxes",
    },
    {
      emoji: "🎁",
      title: "Tax-Free Withdrawals",
      desc: "Use the money for medical expenses and pay zero taxes. It's the only triple-tax-advantaged account!",
    },
  ];
  ...
  pdf.text(
    `• Your healthcare spending: $${annualSpending.toLocaleString()}/year`,
    25,
    yPos,
  );
  yPos += 7;
  pdf.text(
    `• ${CURRENT_TAX_YEAR} HSA max: $${hsaMaxContribution.toLocaleString()} (${data.householdSize === 1 ? "individual" : "family"})`,
    25,
    yPos,
  );
  yPos += 7;
  pdf.text(
    `• Recommended contribution: $${potentialContribution.toLocaleString()}`,
    25,
    yPos,
  );
  yPos += 7;
  ...
  yPos += 7;
  pdf.text(
    `• FICA savings (7.65%): $${Math.round(potentialContribution * 0.0765).toLocaleString()}`,
    25,
    yPos,
  );
  yPos += 12;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `Total Tax Benefit: $${(savings.taxSavings + Math.round(potentialContribution * 0.0765)).toLocaleString()}/year`,
    25,
    yPos,
  );
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("Let's Talk Credit Cards (The Fun Part!)", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "Look, most people are leaving free money on the table. Here's how to pick it up:",
    20,
    yPos,
  );
  yPos += 18;
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "Always pay the full balance monthly. Interest charges will wipe out any rewards!",
    48,
    yPos,
  );
  yPos += 10;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `With your $${annualSpending.toLocaleString()} in spending, that's $${savings.rewardsSavings.toLocaleString()} back in your pocket!`,
    20,
    yPos,
  );
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("The Calendar Hack Nobody Tells You", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "When you spend matters almost as much as how much you spend. Let me break it down:",
    20,
    yPos,
  );
  yPos += 18;
  ...
  pdf.setFont(undefined, "bold");
  pdf.text(
    `Your timing savings: $${savings.timingSavings.toLocaleString()}/year`,
    20,
    yPos,
  );
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("Your 30-Day Implementation Roadmap", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "Forget complicated 12-month plans. Here's what to do in the next 30 days to capture 80% of the value:",
    20,
    yPos,
  );
  yPos += 18;
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("Mistakes That Cost People Thousands", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setFont(undefined, "normal");
  pdf.text(
    "I've seen these mistakes cost people $5k-$15k per year. Don't let this be you:",
    20,
    yPos,
  );
  yPos += 18;
  ...
  pdf.setFontSize(22);
  pdf.setFont(undefined, "bold");
  pdf.text("Questions Everyone Asks Me", pageWidth / 2, 28, {
    align: "center",
  });
  ...
  pdf.setTextColor(brand.secondary.r, brand.secondary.g, brand.secondary.b);
  pdf.text(
    "They'll read it, think 'that's interesting,' and go back to overpaying for healthcare.",
    20,
    yPos + 19,
  );
  yPos += 26;
  ...
  pdf.setFontSize(18);
  pdf.text(
    `You're leaving $${savings.total.toLocaleString()} on the table this year`,
    pageWidth / 2,
    yPos + 24,
    { align: "center" },
  );
