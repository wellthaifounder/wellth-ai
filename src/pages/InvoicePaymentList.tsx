...
const InvoicePaymentList = () => {
  ...
  const getEligibilityStatus = (invoice: any) => {
    ...
    if (hsaOpenedDate) {
      ...
      if (!isNaN(invoiceDate.getTime()) && invoiceDate < hsaDate)
        return "pre-hsa";
    }
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return invoices.filter(
      (invoice) => getEligibilityStatus(invoice) === eligibilityFilter,
    );
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-5 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                ${aggregateStats.totalInvoiced.toFixed(2)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                ${aggregateStats.totalPaidHSA.toFixed(2)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">
                ${aggregateStats.totalPaidOther.toFixed(2)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">
                ${aggregateStats.totalUnpaid.toFixed(2)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-primary/5 border-primary/20">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-primary">
                ${aggregateStats.totalHSAEligible.toFixed(2)}
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-yellow-900 dark:text-yellow-100">
                    {aggregateStats.preHSACount} expense
                    {aggregateStats.preHSACount > 1 ? "s" : ""} occurred before
                    your HSA was opened
                  </p>
                  <p className="text-sm text-yellow-800 dark:text-yellow-200 mt-1">
                    ${aggregateStats.preHSAAmount.toFixed(2)} in expenses are
                    not eligible for HSA reimbursement because they occurred
                    before{" "}
                    {hsaOpenedDate
                      ? new Date(hsaOpenedDate).toLocaleDateString()
                      : "your HSA opened"}
                    .
                  </p>
                </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Track bill balances and payment history for HSA reimbursement
                  optimization
                </CardDescription>
              </div>
              <Select
              ...
              <div className="text-center py-12">
                <p className="text-muted-foreground mb-4">
                  {eligibilityFilter === "all"
                    ? "No invoices yet"
                    : `No ${eligibilityFilter === "eligible" ? "HSA eligible" : eligibilityFilter === "pre-hsa" ? "pre-HSA" : "ineligible"} invoices found`}
                </p>
                <Button onClick={() => navigate("/invoice/new")}>
                  <Plus className="h-4 w-4 mr-2" />
                                  ...
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1">
                                      <p className="font-semibold text-primary">
                                        HSA Reimbursement Opportunity
                                      </p>
                                      <p className="text-sm text-muted-foreground mt-1">
                                        <span className="font-semibold">
                                          $
                                          {breakdown.hsaReimbursementEligible.toFixed(
                                            2,
                                          )}
                                        </span>{" "}
                                        eligible for reimbursement
                                        {breakdown.alreadyPaidRecoverable >
                                          0 && (
                                          <>
                                            {" "}
                                            ($
                                            {breakdown.alreadyPaidRecoverable.toFixed(
                                              2,
                                            )}{" "}
                                            already paid, recoverable)
                                          </>
                                        )}
                                        {breakdown.unpaidStrategicOpportunity >
                                          0 && (
                                          <>
                                            {" "}
                                            + $
                                            {breakdown.unpaidStrategicOpportunity.toFixed(
                                              2,
                                            )}{" "}
                                            unpaid (pay with rewards card, then
                                            reimburse!)
                                          </>
                                        )}
                                      </p>
                            ...
                            {invoice.payment_transactions &&
                              invoice.payment_transactions.length > 0 && (
                                <div>
                                  ...
                                  <div className="space-y-2">
                                    {invoice.payment_transactions.map(
                                      (payment: any) => (
                                        <div
                                          key={payment.id}
                            ...
                            <div className="flex gap-2 pt-2">
                              <Button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(
                                    `/payment/new?invoice=${invoice.id}`,
                                  );
                                }}
                              >
