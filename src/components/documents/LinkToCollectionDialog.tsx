...
export function LinkToCollectionDialog({
  ...
  const createAndLinkMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["documents"] });
      queryClient.invalidateQueries({ queryKey: ["collections"] });
      toast.success("Care event created and document linked");
      onSuccess();
      onOpenChange(false);
    },
    onError: () => {
  ...
  return (
    <ResponsiveDialog open={open} onOpenChange={onOpenChange}>
      <ResponsiveDialogHeader>
        <ResponsiveDialogTitle>Link to Care Event</ResponsiveDialogTitle>
        <ResponsiveDialogDescription>
          Connect this document to a care event for better organization.
        </ResponsiveDialogDescription>
      </ResponsiveDialogHeader>

      <ResponsiveDialogBody className="px-4 pb-4 overflow-y-auto space-y-4">
        ...
        {showNewCollection ? (
          <div className="space-y-2">
            <Label htmlFor="new-collection-title">Care Event Title</Label>
            <Input
            ...
            <p className="text-xs text-muted-foreground">
              You can add more details after creating the care event.
            </p>
          </div>
        ) : (
          <>
      ...
      <ResponsiveDialogFooter>
        ...
        >
          ...
          {showNewCollection ? "Create & Link" : "Link to Care Event"}
        </Button>
      </ResponsiveDialogFooter>
    </ResponsiveDialog>
  );
