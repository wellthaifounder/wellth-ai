...
export function DisputeCommunicationLog({
  disputeId,
  communications,
  onRefresh,
}: DisputeCommunicationLogProps) {
        ...
        {communications.length === 0 ? (
          ...
          <div className="space-y-3">
            {communications.map((comm) => {
              const Icon =
                communicationIcons[
                  comm.communication_type as keyof typeof communicationIcons
                ];
              const DirectionIcon =
                comm.direction === "outbound" ? ArrowRight : ArrowLeft;
                    ...
                    <div className="flex-1">
                      ...
                      {comm.follow_up_required && comm.follow_up_date && (
                        <Badge variant="secondary" className="mt-2">
                          Follow-up:{" "}
                          {format(new Date(comm.follow_up_date), "MMM d, yyyy")}
                        </Badge>
                      )}
                    </div>
                  </div>
