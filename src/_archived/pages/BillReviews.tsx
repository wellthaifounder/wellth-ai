...
export default function BillReviews() {
  ...
  const filteredReviews = useMemo(() => {
    ...
    return reviewsData.filter((review) => {
      ...
      if (filters.searchQuery) {
        ...
        const matchesVendor = review.invoices?.vendor
          ?.toLowerCase()
          .includes(query);
      ...
      if (
        ...
        Number(review.total_potential_savings) < filters.savingsMin
      ) {
        return false;
      }
  ...
  const pendingReviews =
    filteredReviews?.filter((r) => r.review_status === "pending") || [];
  const resolvedReviews =
    filteredReviews?.filter((r) => r.review_status === "resolved") || [];
  const totalSavings =
    filteredReviews?.reduce(
      (sum, r) => sum + Number(r.total_potential_savings),
      0,
    ) || 0;
